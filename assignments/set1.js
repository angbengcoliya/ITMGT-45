...
function savings(grossPay, taxRate, expenses) {
    let afterTax = Math.floor(grossPay * (1 - taxRate));
...
function materialWaste(totalMaterial, materialUnits, numJobs, jobConsumption) {
    let remainingMaterial = totalMaterial - (numJobs * jobConsumption);
 ...
 *
 * To get simple interest, simply multiply the principal to the quantity (rate * time). Add this amount to the principal to get the final value.
 *
...
function interest(principal, rate, periods) {
    let finalAmount = principal + (principal * rate * periods);
